...
class KubernetesCompute(Compute):
    ...
    def check_volumes(self, volumes, namespace):
        try:
            volumesNA = []
            api_response = self.api.list_namespaced_persistent_volume_claim(
                namespace=namespace)
    ...
    def is_ambassador_context(self, namespace):
        try:
            api_response = self.api.list_namespaced_service(
                field_selector="metadata.name=ambassador", namespace=namespace)
            if len(api_response.items) > 0:
                return True
            else:
                return False
        except ApiException as e:
            logger.info(f"Ambassador is not configured.")
    ...
    def start(self, system, namespace="default"):
        """ Start an abstractly described distributed system on the cluster.
            ...
            explicitly modeled elements include Deployment,
        ...
        try:
            ...
            try:
                api_response = self.api.list_namespaced_secret(
                    namespace=namespace)
                for item in api_response.items:
                ...
                logger.debug(
                    f"App requires {system.system_name}-env configmap with envs: {e}")
            ...
            if system.requires_network_policy():
                ...
                for network_policy_manifest in network_policy_manifests:
                    logger.debug(
                        f"applying network policy: {network_policy_manifest}")
            ...
            for container in system.containers:
                ...
                if service:
                    logger.debug(
                        f"generating service for container {container.name}")
                    service_manifests = system.render(
                        template="service.yaml",
                        context={
                            "service": service, "create_deployment_api_response": create_deployment_api_response}
                    )
                    for service_manifest in service_manifests:
                        logger.debug(
                            f"-- creating service for container {container.name}")
                        response = self.api.create_namespaced_service(
                            body=service_manifest,
                            namespace=namespace)

                        # We can't confidently know the Service IP address now since we'd be racing the k8s CNI/LoadBalancer provider for that information.
                        # Return localhost since client.py dies if it receives None
    ...
    def delete(self, name, namespace="default"):
        ...
        try:
            ...
            for object_type, finalizer in finalizers.items():
                logger.debug(
                    f" --deleting {object_type} elements of {name} in namespace {namespace}")
                response = finalizer(
    ...
    def status(self, name=None, username=None, namespace="default"):
        ...
        if response is not None:
            for item in response.items:
                ...
                workspace_name = item.spec.template.metadata.labels.get(
                    "app-name", "")
    ...
    def modify(self, system_modify):
        ...
        try:
            ...
            if len(api_response) == 0:
                raise Exception(
                    "No deployments found. Specify a valid GUID. Format {'guid': '<name>'}.")
            ...
            for deployment in api_response:
                ...
                templates = list(generator.render(
                    "patch.yaml", context={"system_modify": system_modify}))
